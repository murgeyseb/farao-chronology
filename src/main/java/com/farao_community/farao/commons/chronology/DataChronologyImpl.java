...
public final class DataChronologyImpl<T extends Object> implements DataChronology<T> {
    ...
    private void store(T data, Interval intervalToStore) {
        if (storedIntervals.keySet().stream().anyMatch(interval -> interval.overlaps(intervalToStore))) {
            throw new ChronologyException("A data is already provided for some instant of the interval");
        }
    ...
    @Override
    public void storeDataAtInstant(T data, Instant instant, Duration duration) throws ChronologyException {
        store(data, Interval.of(instant, duration));
    }

    @Override
    public void storeDataAtInstant(T data, Instant instant, Period period) throws ChronologyException {
